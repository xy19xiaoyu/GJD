class SubOrdersController < ApplicationController
  ...
  def create
    ...
    respond_to do |format|
      if @sub_order.save
        format.html { redirect_to Order.find_by_OrderId(@sub_order.OrderId), notice: 'Sub order was successfully created.' }
  ...
  def destroy
    @sub_order.destroy
    respond_to do |format|
      format.html { redirect_to Order.find_by_OrderId(@sub_order.OrderId), notice: 'Sub order was successfully destroyed.' }
  ...
  private
    ...
    def sub_order_params
      params.require(:sub_order).permit(:OrderId, :ItemNo, :CreateTime, :ShelfLife, :Sum, :Sum1, :BatchId, :MadeIn,:GoDownId)
    end
end
